...
class ConstantPad1d(Function):
    ...
    def forward(ctx, input, target_size, dimension=0, value=0, pad_start=False):
        num_pad = target_size - input.shape[dimension]
        assert num_pad >= 0, 'target size has to be greater than input size'
        ctx.save_for_backward(num_pad)
        ctx.save_for_backward(dimension)
        ctx.save_for_backward(torch.tensor(pad_start))

        input_size = input.shape
        ctx.save_for_backward(torch.tensor(input_size))
        ...
        # crop output
        if pad_start:
            c_output = c_output.narrow(dimension, num_pad, c_output.shape[dimension] - num_pad)
        else:
            c_output = c_output.narrow(dimension, 0, c_output.shape[dimension] - num_pad)
    ...
    def backward(ctx, grad_output):
        num_pad, dimension, pad_start, input_size = ctx.saved_tensors
