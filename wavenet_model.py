...
# Borrowed from https://github.com/vincentherrmann/pytorch-wavenet/tree/master
...
class WaveNetModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.speaker_embeddings_f = nn.ModuleList()
        self.speaker_embeddings_g = nn.ModuleList()
        ...
        for b in range(blocks):
            ...
            for i in range(layers):
                ...
                # Speaker embeddings filter
                self.speaker_embeddings_f.append(nn.Linear(in_features=speaker_embeddings_size,
                ...
                # Speaker embeddings gate
                self.speaker_embeddings_g.append(nn.Linear(in_features=speaker_embeddings_size,
                                                           out_features=residual_channels))
    ...
    def wavenet(self, input, dilation_func, speaker_id):
        ...
        for i in range(self.blocks * self.layers):
            ...
            speaker_embedding_f = self.speaker_embeddings_f[i](speaker_id)[:, :, None].expand(-1, -1, filter.shape[-1])
            print("Filter shape : %s" %str(filter.shape))
            filter = F.tanh(filter + speaker_embedding_f)
            gate = self.gate_convs[i](residual)
            speaker_embedding_g = self.speaker_embeddings_g[i](speaker_id)[:, :, None].expand(-1, -1, gate.shape[-1])
            gate = F.sigmoid(gate + speaker_embedding_g)
            x = filter * gate
    ...
    def forward(self, input, speaker_id):
        x = self.wavenet(input, dilation_func=self.wavenet_dilate, speaker_id=speaker_id)
